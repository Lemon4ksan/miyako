...
func TestBus_BaseEventAndClosedSub(t *testing.T) {
	// 1. Test BaseEvent isEvent
	var ev BaseEvent
	ev.isEvent()

	// 2. Test BaseEvent Context (nil Ctx)
	ctx := ev.Context()
	assert.NotNil(t, ctx)
	assert.Equal(t, context.Background(), ctx)

	// 3. Test BaseEvent Context (non-nil Ctx)
	...
	// 4. Test closed Bus Subscribe & SubscribeAll
	b := New()
	_ = b.Close()
	...
	// 5. Test Subscribe with nil events
	b2 := New()
	defer b2.Close()
